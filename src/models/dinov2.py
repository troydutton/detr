...
class Dinov2WithRegistersEncoder(Dinov2WithRegistersPreTrainedModel):
    def __init__(self, config: Dinov2WithRegistersConfig, out_feature_indices: list[int] | None = None):
        super().__init__()
    ...
    def forward(self, hidden_states: torch.Tensor, **kwargs) -> list[torch.Tensor]:
        features = []
...
class Dinov2WithRegistersModel(Dinov2WithRegistersPreTrainedModel):
    def __init__(self, config: Dinov2WithRegistersConfig, out_feature_indices: list[int] | None = None):
        ...
        self.encoder = Dinov2WithRegistersEncoder(config, out_feature_indices=out_feature_indices)
    ...
    def forward(
        ...
        **kwargs,
        ...
        output_features = []
        batch_size, _, height, width = pixel_values.shape
        h, w = height // self.config.patch_size, width // self.config.patch_size
        ...
        for sequence_output in encoder_outputs:
            ...
            feature = patch_tokens.transpose(1, 2).reshape(batch_size, self.config.hidden_size, h, w)
