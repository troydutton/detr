...
class TransformerDecoder(Module):
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            layer_class_logits: Tensor = self.class_head(query_embed)
            ...
            if i == 0:
                ...
                # Treat the initial references as fixed priors for all subsequent layers
                initial_references = layer_boxes.detach()
                layer_class_logits = layer_class_logits.detach()

            # Refine the edge offsets and boxes
            layer_edge_logits = layer_edge_logits + self.edge_head(query_embed)
            layer_boxes = add_edge_offset(initial_references, layer_edge_logits, self.edge_offset_weights)

            # Refine the class logits with edge information
            class_offsets = self.edge_class_head(torch.cat([query_embed, layer_edge_logits], dim=-1))
            layer_class_logits = layer_class_logits + class_offsets

            # Update the reference boxes and positional embeddings for the next layer
